...
// Drop-in replacement for `blockTranslations` in src/components/dashboard-client.tsx.
// `headline` and `subtitle` keep their existing meaning, so existing render code
// keeps working unchanged. `examples`, `notHere`, `tip` and `placeholder` are new
// and optional — render them when you are ready (see BlockCard notes in the review).
